...
function start(r,l) {
    ...
    // rem tells how many pairs are there in the matrix.
...
function change(x) {
  //Variables
  // whatever block x is pressed we apply certain css effects, as the The onclick event occurs when the user clicks on an element line 59.
  ...
  //Flip
    ...
    if (turn==1) {
      ...
      if (pre!=$(b).text()) {
         setTimeout(function() {
            // t=transform , fliping it back as they are not same
            $(pID).css(t, flipBack);
            $(i).css(t, flipBack);
            ppID=0;
         },1000);
    ...
    // when turn =0 we come here.
    else {
        // setting previos block to pre, the next block which has to be openend for a match to ppID
      pre = $(b).text(); // stored the 1st block icon  
      ppID = x; 
      pID = "#"+x+" .inner";
      turn=1;
    }
